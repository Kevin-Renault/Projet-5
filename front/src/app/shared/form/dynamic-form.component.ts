...
export class DynamicFormComponent implements OnChanges {
    ...
    getErrorMessage(controlName: string): string {
        ...
        if (control.hasError('required')) return `${formElement.name} est obligatoire.`;
        if (control.hasError('minlength')) return `${formElement.name} est trop court (min ${control.errors!['minlength'].requiredLength}).`;
        if (control.hasError('maxlength')) return `${formElement.name} est trop long (max ${control.errors!['maxlength'].requiredLength}).`;
        if (control.hasError('pattern')) return `Le format de ${formElement.name} est invalide.`;
        return '';
    }
}
